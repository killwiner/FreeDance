...
void ExportMotion::save(const QString &fileName,const Skeleton &skel, const int &nbr_frames) {
    ...
    try {
        // open the file
        ofstream file_try(fileName.toStdString().c_str(), ofstream::out);
        if (!file_try)
    ...
    *file << "HIERARCHY\n";
    ...
    write_new_root(*file, string("ROOT hips"), skel.hips->s, false, true);
    ...
    write_new_root(*file, string("JOINT hand_R"), skel.hand_r->s, false, true); // 14
    ...
    write_new_root(*file, string("JOINT hand_L"), skel.hand_l->s, false, true); // 22
    ...
    write_close_root(*file);
    write_close_root(*file);
    write_new_root(*file, string("JOINT neck"), skel.neck->s, false, true); // 26
    write_new_root(*file, string("JOINT head"), skel.head->s, false, false); // 27
